    ...
    if (order.params.type === "buy") {
        ...
        if (options === null || options === void 0 ? void 0 : options.onChainApprovalRecheck) {
            if ((0, utils_1.bn)(await onChainData
                .fetchAndUpdateFtApproval(order.params.currency, order.params.maker, Sdk.X2Y2.Addresses.Exchange[index_1.config.chainId])
        ...
        // Check: maker has enough balance
        const nftBalance = await commonHelpers.getNftBalance(order.params.nft.token, order.params.nft.tokenId, order.params.maker);
        if (nftBalance.lt(1)) {
            hasBalance = false;
        }
        const operator = Sdk.X2Y2.Addresses.Erc721Delegate[index_1.config.chainId];
        // Check: maker has set the proper approval
        const nftApproval = await commonHelpers.getNftApproval(order.params.nft.token, order.params.maker, operator);
        if (!nftApproval) {
            if (options === null || options === void 0 ? void 0 : options.onChainApprovalRecheck) {
                // Re-validate the approval on-chain to handle some edge-cases
                const contract = kind === "erc721"
                    ? new Sdk.Common.Helpers.Erc721(provider_1.baseProvider, order.params.nft.token)
                    : new Sdk.Common.Helpers.Erc1155(provider_1.baseProvider, order.params.nft.token);
